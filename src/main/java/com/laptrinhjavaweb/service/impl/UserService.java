...
public class UserService implements IUserService {
    ...
    @Override
    public List<StaffResponseDTO> assignmentStaffs(Long id, String name) {
        List<UserEntity> staffs = userRepository.findByStatusAndRoles_Code(1, SystemConstant.STAFF);
        ...
        for (UserEntity item: staffs){
            ...
            if(name.equals("building")){
                isExist = buildingRepository.existsByIdAndStaffs_Id(id, item.getId());
            }
            if (name.equals("customer")){
                isExist = customerRepository.existsByIdAndCustomerStaffs_Id(id, item.getId());
            }
            if (isExist){
                staffResponseDTO.setChecked((SystemConstant.CHECKED));
            }else {
                staffResponseDTO.setChecked((SystemConstant.EMPTY));
            }
